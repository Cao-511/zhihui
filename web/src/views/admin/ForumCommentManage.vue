...
<!--          <el-button type="danger" :icon="Delete" @click="batchDelete(null)" :disabled="selectionRows.length<=0">批量删除</el-button>-->
      ...
      <el-card>
        <el-table ref="tableComponents"
          ...
          <el-table-column prop="forumName" label="论坛标题"></el-table-column>
...
function submit() {
  formRef.value.validate((valid) => {
    ...
    if (!formData.value.id) {
      ...
      //更新
      request.put("/forumComment/update", formData.value).then(res => {
        if (!res) {
          return
        }
        dialogOpen.value = false
...
function batchDelete(rows) {
  ...
  ElMessageBox.confirm(`此操作将永久删除ID为[${ids}]的数据, 是否继续?`, '提示', {
    ...
    request.delete("/forumComment/delBatch", {data: ids}).then(res => {
      if (!res) {
        return
      }
      ElMessage({
