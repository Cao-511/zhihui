  ...
  <div>
    <el-space direction="vertical" alignment="left" style="width: 100%">
      ...
      <el-card>
        <el-table ref="tableComponents"
          ...
          <el-table-column show-overflow-tooltip  prop="name" label="停车场名称"></el-table-column>
          <el-table-column show-overflow-tooltip  prop="address" label="停车场地址"></el-table-column>
          <el-table-column show-overflow-tooltip  prop="parkingNum" label="车位数量"></el-table-column>
          <el-table-column show-overflow-tooltip  prop="status" label="停车场状态">
    ...
    >
      <el-form ref="formRef" :model="formData" label-width="100px">
        <slot name="content">
          <el-form-item label="停车场名称" prop="name"  :rules="[{required:true,message:'不能为空',trigger:[ 'blur','change']}]">
            <el-input v-model="formData.name"></el-input>
          </el-form-item>
          <el-form-item label="停车场地址" prop="address"  :rules="[{required:true,message:'不能为空',trigger:[ 'blur','change']}]">
            <el-input v-model="formData.address"></el-input>
          </el-form-item>
          <el-form-item label="车位数量" prop="parkingNum"  :rules="[{required:true,message:'不能为空',trigger:[ 'blur','change']}]">
            <el-input type="number" v-model="formData.parkingNum"></el-input>
          </el-form-item>
          <el-form-item label="停车场状态" prop="status">
...
function submit() {
  formRef.value.validate((valid) => {
    ...
    if (!formData.value.id) {
      request.post("/parkingLot/add", formData.value).then(res => {
        if (!res||res.code!==200) {
          ElMessage({
            message: res.data,
            type: 'error'
      ...
      //更新
      request.put("/parkingLot/update", formData.value).then(res => {
        if (!res) {
          return
        }
        dialogOpen.value = false
...
function batchDelete(rows) {
  ...
  ElMessageBox.confirm(`此操作将永久删除ID为[${ids}]的数据, 是否继续?`, '提示', {
    ...
    request.delete("/parkingLot/delBatch", {data: ids}).then(res => {
      if (!res) {
        return
      }
      ElMessage({
