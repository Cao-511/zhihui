      ...
      <el-card>
        <el-table ref="tableComponents"
          ...
          <el-table-column prop="status" label="状态">
            <template #default="scope">
              <el-tag type="success" v-if="scope.row.status==='已处理'">已支付</el-tag>
              <el-tag type="danger" v-if="scope.row.status==='未处理'">未处理</el-tag>
            </template>
          </el-table-column>
          <el-table-column width="180px" prop="createTime" label="投诉时间"></el-table-column>
...
<!--              <el-button :icon="Delete" type="danger" @click="deleteOne(scope.$index, scope.row)">删除</el-button>-->
    ...
    >
      <el-form ref="formRef" :model="formData" label-width="100px">
        <slot name="content">
          <el-form-item label="投诉类型" prop="type"  :rules="[{required:true,message:'不能为空',trigger:[ 'blur','change']}]">
...
const typeList = ref(['设施故障','环境卫生','噪音扰民','占道经营','停车管理'])
...
function submit() {
  formRef.value.validate((valid) => {
    ...
    if (!formData.value.id) {
      request.post("/complaint/add", formData.value).then(res => {
        if (!res) {
          return
        }
        dialogOpen.value = false
      ...
      //更新
      request.put("/complaint/update", formData.value).then(res => {
        if (!res) {
          return
        }
        dialogOpen.value = false
...
function batchDelete(rows) {
  ...
  ElMessageBox.confirm(`此操作将永久删除ID为[${ids}]的数据, 是否继续?`, '提示', {
    ...
    request.delete("/complaint/delBatch", {data: ids}).then(res => {
      if (!res) {
        return
      }
      ElMessage({
