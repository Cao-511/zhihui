    ...
    <el-space direction="vertical" alignment="left" style="width: 100%">
      ...
      <el-card>
        <el-table ref="tableComponents"
          ...
          <el-table-column prop="type" label="收费类型"></el-table-column>
          <el-table-column prop="reading" label="读数">
            <template #default="{row}">
              {{ row.reading }} {{ row.type === '水费' ? '吨' : '度' }}
            </template>
          </el-table-column>
          ...
          <el-table-column fixed="right" label="操作" width="200">
            <template #default="scope">
              <el-button type="primary" link v-if="scope.row.paymentStatus==='未支付'" @click="pay(scope.row)">付款</el-button>
            </template>
          </el-table-column>
        </el-table>
    ...
    >
      <el-form ref="formRef" :model="formData" label-width="100px">
        <slot name="content">
          ...
          <el-form-item label="收费类型" prop="type" :rules="[{required:true,message:'不能为空',trigger:[ 'blur','change']}]">
...
function getPageList() {
  ...
  request.get("/utilityBillFee/page", {
    params: data
  }).then(res => {
...
function submit() {
  formRef.value.validate((valid) => {
    ...
    if (!formData.value.id) {
      request.post("/utilityBillFee/add", formData.value).then(res => {
        if (!res) {
          return
        }
        dialogOpen.value = false
      ...
      //更新
      request.put("/utilityBillFee/update", formData.value).then(res => {
        if (!res) {
          return
        }
        dialogOpen.value = false
...
function batchDelete(rows) {
  ...
  ElMessageBox.confirm(`此操作将永久删除ID为[${ids}]的数据, 是否继续?`, '提示', {
    ...
    request.delete("/utilityBillFee/delBatch", {data: ids}).then(res => {
      if (!res) {
        return
      }
      ElMessage({
...
function pay(row) {
  request.post("/utilityBillFee/pay/" + row.id).then(res => {
    if (!res) {
      return
    }
    ElMessage({
