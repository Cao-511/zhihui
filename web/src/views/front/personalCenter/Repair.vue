...
<!--          <el-button type="danger" :icon="Delete" @click="batchDelete(null)" :disabled="selectionRows.length<=0">批量删除</el-button>-->
      ...
      <el-card>
        <el-table ref="tableComponents"
          ...
          <el-table-column width="180px" prop="createTime" label="报修时间"></el-table-column>
...
<!--              <el-button :icon="Delete" type="danger" @click="deleteOne(scope.$index, scope.row)">删除</el-button>-->
    ...
    >
      <el-form ref="formRef" :model="formData" label-width="100px">
        <slot name="content">
          <el-form-item label="楼宇房屋" prop="houseId" :rules="[{required:true,message:'不能为空',trigger:[ 'blur','change']}]">
            <el-select v-model="formData.houseId" placeholder="请选择" clearable filterable @change="buildingChange" style="width: 150px">
              <el-option :label="item.houseNumber"  :value="item.id" :key="item" v-for="item in treelist"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="紧急程度" prop="urgencyLevel"  :rules="[{required:true,message:'不能为空',trigger:[ 'blur','change']}]">
            <el-radio-group v-model="formData.urgencyLevel" :rules="[{required:true,message:'不能为空',trigger:[ 'blur','change']}]">
              <el-radio :label="item" :key="item" v-for="item in urgencyLevelList">{{ item }}</el-radio>
            </el-radio-group>
          </el-form-item>
          <el-form-item label="预约时间" prop="appointmentTime" :rules="[{required:true,message:'请选择预约时间',trigger:['blur','change']}]">
            <el-date-picker
          ...
          <el-form-item label="描述" prop="content"  :rules="[{required:true,message:'不能为空',trigger:[ 'blur','change']}]">
...
function getMyTreelist() {
  request.get("/house/mylist").then(res => {
