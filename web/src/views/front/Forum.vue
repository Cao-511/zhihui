    ...
    <el-space direction="vertical" alignment="left" style="width: 100%">
      ...
      >
        ...
        <template #footer>
          ...
          <el-button link type="plan" @click="router.push('/forumDetails/'+item.id)">查看详情</el-button>
...
function submit() {
  formRef.value.validate((valid) => {
    ...
    if (!formData.value.id) {
      request.post("/forum/add", formData.value).then(res => {
        if (!res) {
          return
        }
        dialogOpen.value = false
      ...
      //更新
      request.put("/forum/update", formData.value).then(res => {
        if (!res) {
          return
        }
        dialogOpen.value = false
...
function batchDelete(rows) {
  ...
  ElMessageBox.confirm(`此操作将永久删除ID为[${ids}]的数据, 是否继续?`, '提示', {
    ...
    request.delete("/forum/delBatch", {data: ids}).then(res => {
      if (!res) {
        return
      }
      ElMessage({
...
function toggleChat(item) {
  ...
  request.get("/forumComment/selectByForumId/" + item.id).then(res => {
...
function addComment(item) {
  ...
  request.post("/forumComment/add", formData.value).then(res => {
    if (!res) {
      return
    }
    // 更新评论区列表
